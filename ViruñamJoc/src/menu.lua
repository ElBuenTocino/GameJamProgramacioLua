...
EnemyPlayable = EnemyPlayable or require "src/enemyPlayeable"
Menu = Actor:extend()
...
function Menu:new(image)
    ...
    self.twoPlayerButton = {x = 450, y = 500, w = 100, h = 30}
    self.exitButton  = {x = 450,  y = 550,  w = 100, h = 30}
...
function Menu:drawButtons()
    ...
    love.graphics.rectangle("fill", self.twoPlayerButton.x, self.twoPlayerButton.y, self.twoPlayerButton.w, self.twoPlayerButton.h)
    ...
    love.graphics.print("Two Players", self.twoPlayerButton.x + 10, self.twoPlayerButton.y)
...
function Menu:click(mouse) -- ver si el usuario esta clicando yupii  
    ...
    if love.mouse.isDown(1) and self:clickedTwoPlayer(x,y) then
        for i = 1, #actorList, 1 do
            if (actorList[i] ~= nil) then
                if (actorList[i]:is(Menu)) then
                    table.remove(actorList, i)
        ...
        local s = Player()
        table.insert(actorList,s)
        local ep = EnemyPlayable()
        table.insert(actorList, ep)
        local spFoco = Spawner(5, 2)
        table.insert(actorList,spFoco)
        local spFood = Spawner(1, 1)
        table.insert(actorList,spFood)
...
function Menu:clickedTwoPlayer(x, y)
    return x > self.twoPlayerButton.x and x < self.twoPlayerButton.x + self.twoPlayerButton.w and
           y > self.twoPlayerButton.y and y < self.twoPlayerButton.y + self.twoPlayerButton.h  
end
